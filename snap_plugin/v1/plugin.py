...
_flag_fields = ["name", "flag_type", "description", "default", "json_name"]
_flag_defaults = dict(default=None, json_name=None)

def _make_flagtype():
    def _required_fields(cls):
        l = [k for k in _flag_fields if k not in _flag_defaults]
        return l
    def _optional_fields(cls):
        l = [k for k in _flag_defaults]
        return l

    flagtype = namedtuple("Flag", _flag_fields)
    # copy default values in order of fields in tuple
    flagtype.__new__.__defaults__ = tuple(_flag_defaults[k] for k in flagtype._fields if k in _flag_defaults)

    flagtype._required_fields = _required_fields
    flagtype._optional_fields = _optional_fields
    return flagtype
        ...
        if item.name not in self._flags:
            if item.default is not None:
                if item.default is not False:
                    item = item._replace(description=item.description + " (default: %(default)s)")
            ...
            if isinstance(flag, Flag):
                ...
                try:
                    ...
                    raise TypeError("Invalid arguments to initialize a flag definition, expect ({0} [, {1}]) but got {3}"
                        .format(", ".join(item._required_fields()),
                        ", ".join(item._optional_fields()), flag))
